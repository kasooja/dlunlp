 ...
 * @author K
 */
public class SuggDataIterator1 implements DataSetIterator {
	/**
	 * 
	 */
	 ...
	 */
	public SuggDataIterator1(String dataDirectory, WordVectors wordVectors, int batchSize, int truncateLength, boolean train) throws IOException {
		this.batchSize = batchSize;
		this.vectorSize = 2 * wordVectors.lookupTable().layerSize();
	...
	private DataSet nextDataSet(int num) throws IOException {
		...
		for( int i=0; i<reviews.size(); i++ ){
			...
			for( int j=0; j<tokens.size() && j<maxLength; j++ ){
				String token = tokens.get(j);
				INDArray vector = wordVectors.getWordVectorMatrix(token);
				vector = Nd4j.concat(1, sumVector, vector);
